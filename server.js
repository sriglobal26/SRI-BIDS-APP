...
const EBN_BIDS = [
  { id:'ebn-884913', name:'EBN — Bid Opportunity Alert for TX (884913)', agency:'Various Texas Agencies', city:'Texas', posted:'2026-09-05', due:'2026-09-20', solicitationNo:'884913', scope:'EnviroBidNet Bid Alert #884913 — New E&I engineering opportunity in Texas. Categories include water/wastewater treatment, SCADA systems, instrumentation and controls, electrical engineering services. Log into EnviroBidNet and search Bid #884913 for full specifications and documents.', url:'https://www.envirobidnet.com', source:'EnviroBidNet', value:'TBD', status:'active', region:'texas' },
  { id:'ebn-884038', name:'Opportunity Home San Antonio — RFQ Environmental Engineering Services (884038)', agency:'Opportunity Home San Antonio', city:'San Antonio, TX', posted:'2026-09-04', due:'2026-09-17', solicitationNo:'884038', scope:'RFQ Addenda 1-2 — Environmental Engineering Services for Opportunity Home San Antonio. Scope includes environmental site assessments, remediation consulting, regulatory compliance, instrumentation monitoring systems. 3 bid specifications available. Search Bid #884038 in EnviroBidNet for documents.', url:'https://www.envirobidnet.com', source:'EnviroBidNet', value:'TBD', status:'active', region:'texas' },
  ...
  { id:'ebn-883613', name:'Texas Facilities Commission — RFQ Engineering Site Services New Public Health Laboratory (883613)', agency:'Texas Facilities Commission (TFC)', city:'Austin, TX', posted:'2026-08-25', due:'2026-09-23', solicitationNo:'883613', scope:'RFQ Addendum 1, Q&A — Engineering Site Services for New Public Health Laboratory at 1711 San Jacinto Blvd, Austin TX 78701. Civil Engineering all services. Bid No: 0010042. Includes site surveys, geotechnical, environmental, civil/structural engineering. Search Bid #883613 in EnviroBidNet for full documents.', url:'https://www.envirobidnet.com', source:'EnviroBidNet', value:'TBD', status:'active', region:'texas' },
]
...
async function seedAllBids() {
  const all = [...FIVE_BIDS,...EBN_BIDS,...H2BID_BIDS,...CIVCAST_BIDS,...ESBD_BIDS,...TWDB_BIDS,...MANUAL_BIDS];
  for(const b of all) await saveBid({...b, region:detectRegion(b.city), scrapedAt:new Date().toISOString()});
...
app.post('/api/bids/ebn-ingest', async (req,res)=>{
  try {
    ...
    const fullUrlM = combined.match(/https?:\/\/(?:www\.)?envirobidnet\.com\/subscriber_view_bid\/[^\s"<>]+/i);
    const directUrl = fullUrlM ? fullUrlM[0].trim() : '';
    ...
    if(!bidNum&&!directUrl) return res.json({success:true,skipped:true,reason:'No bid number found'});
    const id = 'ebn-'+(bidNum||Date.now());
    const dup = await pool.query('SELECT id FROM bids WHERE id=$1',[id]);
    if(dup.rows.length>0){
      if(directUrl){
        const ex = await pool.query('SELECT data FROM bids WHERE id=$1',[id]);
        if(ex.rows.length>0){const bid=ex.rows[0].data;bid.url=directUrl;await pool.query('UPDATE bids SET data=$1 WHERE id=$2',[JSON.stringify(bid),id]);}
      }
    ...
    let due='';
    if(dateM){try{const dt=new Date(dateM[1]);if(!isNaN(dt)&&dt.getFullYear()>=2026)due=dt.toISOString().split('T')[0];}catch(e2){}}
    if(due&&new Date(due)<new Date()) return res.json({success:true,skipped:true,reason:'Expired'});
    const agM = subject.match(/^([^:—\-]+)[:\—\-]/);
    const agency = agM ? agM[1].trim() : 'EnviroBidNet';
    const url = directUrl||(bidNum?'https://www.envirobidnet.com/subscriber_view_bid/'+bidNum:'https://www.envirobidnet.com');
    await saveBid({id,name:subject.trim()||'EBN Bid '+bidNum,agency,city:'Texas',
      posted:new Date().toISOString().split('T')[0],due,scope:emailBody.substring(0,500),url,
      source:'EnviroBidNet',value:'TBD',status:'active',region:'texas',userState:'active',
      scrapedAt:new Date().toISOString()});
    console.log('[EBN] Saved:',id,url.substring(0,60));
...
app.get('/api/dedupe-fedbids', async (req,res)=>{
  try {
    await pool.query("DELETE FROM bids WHERE data->>'source'='FedBids' AND id LIKE 'fedbid-00%'");
    for(const b of FIVE_BIDS) await saveBid({...b,scrapedAt:new Date().toISOString()});
    res.json({success:true,reseeded:FIVE_BIDS.length});
  } catch(e){res.status(500).json({error:e.message});}
});
...
app.get('/api/clean-ebn', async (req,res)=>{
  try {
    await pool.query("DELETE FROM bids WHERE data->>'source'='EnviroBidNet' AND (id LIKE 'ebn-auto-%' OR id LIKE 'ebn-178%')");
    await pool.query("DELETE FROM bids WHERE data->>'source'='EnviroBidNet' AND data->>'due'!='' AND (data->>'due')::date < CURRENT_DATE");
    for(const b of EBN_BIDS) await saveBid({...b,scrapedAt:new Date().toISOString()});
    res.json({success:true,reseeded:EBN_BIDS.length});
  } catch(e){res.status(500).json({error:e.message});}
});
...
app.get('/api/fix-fedbids-urls', async (req,res)=>{
  try {
    const r = await pool.query("SELECT id,data FROM bids WHERE data->>'source'='FedBids'");
    res.json({success:true,fedbids:r.rows.length});
  } catch(e){res.status(500).json({error:e.message});}
});
...
cron.schedule('0 */4 * * *', async ()=>{
  try {
    ...
    console.log('[Cron] Expired old bids');
  } catch(e){console.error('[Cron]',e.message);}
});

app.use((err,req,res,next)=>{console.error('[Err]',err.message);res.status(500).json({error:err.message});});

app.listen(PORT,'0.0.0.0',()=>{
  console.log('[SRI Bids] Running on port',PORT);
  setTimeout(()=>initDB().catch(e=>console.error('[DB]',e.message)),500);
});
